...
class AttributionHeadBert(nn.Module):
  
  def __init__(self, model_path):
    super(AttributionHeadBert,self).__init__() 

    self.transformer = transformers.BertModel.from_pretrained(model_path, is_decoder=False)
    ...
    self.attribution_head = torch.nn.Linear(768, out_features)
  ...
  def load(self, path, path2):
    ...
    self.transformer = transformers.BertModel.from_pretrained(path2)